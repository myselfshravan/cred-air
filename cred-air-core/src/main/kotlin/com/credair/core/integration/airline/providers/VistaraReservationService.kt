...
@Singleton
class VistaraReservationService : AirlineReservationService {
    
    override suspend fun softReserve(request: ReservationRequest): ReservationResponse {
        return try {
            val confirmationCode = generateConfirmationCode("UK")
            val pnr = generatePNR()
            ...
            ReservationResponse(
                success = false,
                error = "Failed to create soft reservation with Vistara: ${e.message}"
            )
        }
    }
    ...
    override suspend fun confirmReservation(airlineConfirmationCode: String, pnr: String): ReservationResponse {
        return try {
            ...
            ReservationResponse(
                success = false,
                error = "Failed to confirm reservation with Vistara: ${e.message}"
            )
        }
    }
    ...
    override suspend fun cancelReservation(request: CancellationRequest): CancellationResponse {
        return try {
            CancellationResponse(
                success = true,
                refundAmount = "Refund as per Club Vistara terms",
                cancellationFee = "₹3,000"
            )
        } catch (e: Exception) {
            CancellationResponse(
                success = false,
                error = "Failed to cancel reservation with Vistara: ${e.message}"
            )
        }
    }
    ...
    override suspend fun getAvailableSeats(flightId: String): List<String> {
        return listOf("21A", "21B", "21C", "22A", "22B", "22C", "23A", "23B", "23C")
    }

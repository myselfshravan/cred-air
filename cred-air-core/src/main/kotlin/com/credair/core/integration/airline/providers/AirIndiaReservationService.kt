...
class AirIndiaReservationService : AirlineReservationService {
    ...
    override suspend fun softReserve(request: ReservationRequest): ReservationResponse {
        return try {
            ...
            ReservationResponse(
                success = false,
                error = "Failed to create soft reservation with Delta Airlines: ${e.message}"
            )
        }
    }
    ...
    override suspend fun confirmReservation(airlineConfirmationCode: String, pnr: String): ReservationResponse {
        return try {
            ...
            ReservationResponse(
                success = false,
                error = "Failed to confirm reservation with Delta Airlines: ${e.message}"
            )
        }
    }
    ...
    override suspend fun cancelReservation(request: CancellationRequest): CancellationResponse {
        return try {
            CancellationResponse(
                success = true,
                refundAmount = "Full refund available within 24 hours",
                cancellationFee = "$0.00"
            )
        } catch (e: Exception) {
            CancellationResponse(
                success = false,
                error = "Failed to cancel reservation with Delta Airlines: ${e.message}"
            )
        }
    }
